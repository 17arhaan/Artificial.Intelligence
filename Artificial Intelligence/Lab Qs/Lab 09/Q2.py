...
def water_jug_dfs(current_state, target, capacities, visited, path):
    ...
    next_states = [
        ...
        ((capacities[0], current_state[1]), "Fill Jug 1"),
        ((current_state[0], capacities[1]), "Fill Jug 2"),
        ((min(current_state[0] + current_state[1], capacities[0]), max(0, current_state[0] + current_state[1] - capacities[0])), "Pour Jug 2 to Jug 1"),
        ((max(0, current_state[0] + current_state[1] - capacities[1]), min(current_state[0] + current_state[1], capacities[1])), "Pour Jug 1 to Jug 2")
    ]

    for next_state, action in next_states:
        if next_state not in visited and is_valid(next_state, capacities):
            new_path = water_jug_dfs(next_state, target, capacities, visited, path + [action])
            if new_path:
                return new_path
...
visited = set()
solution = water_jug_dfs(initial_state, target_state, jug_capacities, visited, [])
